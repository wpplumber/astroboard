<template>
  <div
    v-if="!loading"
    class="tw-relative tw-flex tw-flex-col tw-justify-end tw-bg-white tw-rounded-lg tw-h-[23vh]"
  >
    <div
      class="tw-absolute tw-z-10 tw-top-1 tw-right-1 tw-flex tw-justify-end tw-items-center"
    >
...
const series = ref([]);
const labels = ref(["Published", "Unpublished", "Deleted"]);
...
const fetchData = async () => {
  try {
    ...
    getChartOptions.value.labels = labels.value;
    loading.value = false;
    if (chart) {
