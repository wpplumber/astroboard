...
const getFullChartOptions = computed(() => {
  ...
  return {
    grid: {
      ...
      left: 80,
      right: 15,
      bottom: 30, // Increased to accommodate zoom slider
      containLabel: false,
    },
    graphic: [
      {
    ...
    tooltip: {
      ...
      formatter: (params: any) => {
        return `${params[0].axisValue}<br/>Groups: ${params[0].data}`;
      },
    },
...
const fetchData = async () => {
  ...
  try {
    ...
    const response = await fetch(
      `${props.currentHost}${API_PREFIX}api/astroboard/gettotalgroups?period=${selectedItem.value}`,
    );
    const result = await response.json();

    data.value = result.lastGroupsCreated;
    categories.value = result.lastGroupsCreatedDates;
    rate.value = result.rate;
    value.value = result.totalGroups;
...
onMounted(async () => {
  ...
  if (fullChartRef.value) {
    ...
    fullChart.on("dataZoom", (params) => {
      // You can add custom logic here when zooming occurs
